...
public class IndyInvocationCompiler implements InvocationCompiler {
    ...
    public void invokeOther(String file, int line, String scopeFieldName, CallBase call, int arity) {
        ...
        if (blockPassType.given()) {
            if (arity == -1) {
                compiler.adapter.invokedynamic("invoke:" + JavaNameMangler.mangleMethodName(id), CodegenUtils.sig(JVM.OBJECT, params(ThreadContext.class, JVM.OBJECT, JVM.OBJECT, JVM.OBJECT_ARRAY, Block.class)), NormalInvokeSite.BOOTSTRAP, blockPassType.literal(), file, line);
            } else {
